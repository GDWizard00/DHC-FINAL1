...
export class ExplorationHandler {
    ...
    static async showExplorationResult(interaction, gameState, embed, options) {
        try {
            ...
            // Always use editReply for exploration results to maintain consistency
    ...
    static generateTreasure(floor) {
        const baseGold = Math.floor(Math.random() * (floor * 5)) + (floor * 2);
        const goldAmount = Math.floor(baseGold * (0.8 + Math.random() * 0.4)); // ±20% variance
        ...
        if (Math.random() < 0.2 + (floor * 0.03)) {
            const commonItems = ['health_potion', 'mana_potion', 'scroll_of_healing'];
    ...
    static generateChestReward(floor) {
        const baseGold = Math.floor(Math.random() * (floor * 10)) + (floor * 5);
        const goldAmount = Math.floor(baseGold * (1.2 + Math.random() * 0.6)); // Better than regular treasure
        
        const reward = {
            gold: goldAmount,
            keys: Math.floor(Math.random() * 4) + 2, // 2-5 keys
            description: `💰 ${goldAmount} Gold\n🗝️ ${reward.keys} Keys`
        };
    ...
    static async fleeFromAmbush(interaction, gameState) {
        try {
            // Apply 10% health and mana loss
            const healthLoss = Math.ceil(gameState.hero.health * 0.1);
            const manaLoss = Math.ceil(gameState.hero.mana * 0.1);
            
            gameState.hero.health = Math.max(1, gameState.hero.health - healthLoss);
            gameState.hero.mana = Math.max(0, gameState.hero.mana - manaLoss);

            const embed = new EmbedBuilder()
                .setTitle('🏃 **FLED FROM COMBAT** 🏃')
                .setDescription(`You manage to escape from the ambush, but not without cost!\n\n**Penalties Applied:**\n❤️ Health Lost: ${healthLoss}\n💙 Mana Lost: ${manaLoss}\n\n**Current Status:**\n❤️ Health: ${gameState.hero.health}/${gameState.hero.maxHealth}\n💙 Mana: ${gameState.hero.mana}/${gameState.hero.maxMana}`)
    ...
    static async enterRoom(interaction, gameState, wasLocked) {
        try {
            ...
            if (roomContents.trapDamage > 0) {
                gameState.hero.health = Math.max(1, gameState.hero.health - roomContents.trapDamage);
            }
    ...
    static generateRoomContents(floor, wasLocked) {
        ...
        const baseGold = Math.floor(Math.random() * (floor * 8)) + (floor * 3);
        const goldAmount = Math.floor(baseGold * multiplier);
        ...
        if (Math.random() < 0.3 + (floor * 0.04)) {
            const items = ['health_potion', 'mana_potion', 'scroll_of_healing', 'enchantment_scroll'];
