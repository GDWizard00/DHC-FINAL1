...
export class AuditLogger {
    ...
    startMonitoring() {
        ...
        this.log('AUDIT:MONITOR', 'Automated monitoring started - 15 minute intervals', 'monitoring_start');
    }
    ...
    async runComplianceCheck() {
        try {
            ...
            // Run validation
            const violations = rulesValidator.validateProject();
            
            // Filter out repeated violations within cooldown period
            const filteredViolations = this.filterViolations(violations);
            
            if (filteredViolations.length > 0) {
                this.log('AUDIT:VIOLATION', `Found ${filteredViolations.length} RULES.txt violations`, 'compliance_check');
                
                // Log each unique violation type once
                const violationsByType = new Map();
                filteredViolations.forEach(violation => {
                    if (!violationsByType.has(violation.rule)) {
                        violationsByType.set(violation.rule, []);
                    }
                    violationsByType.get(violation.rule).push(violation);
                });
                
                // Log violations by type
                violationsByType.forEach((violations, ruleType) => {
                    const fileCount = new Set(violations.map(v => v.file)).size;
                    this.log('AUDIT:VIOLATION', 
                        `${ruleType}: ${violations.length} violations in ${fileCount} files`, 
                        'rule_violation'
                    );
                });
                
                // Generate repair recommendations
                const recommendations = rulesValidator.getRepairRecommendations(filteredViolations);
                if (recommendations.length > 0) {
                    this.planRepairs(recommendations);
                }
            } else {
                this.log('AUDIT:COMPLIANCE', 'All RULES.txt compliance checks passed', 'compliance_success');
            }
        } catch (error) {
            this.log('AUDIT:ERROR', `Compliance check failed: ${error.message}`, 'compliance_error');
        }
    }
