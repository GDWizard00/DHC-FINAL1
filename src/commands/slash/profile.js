    ...
    try {
        switch (sub) {
            ...
            case 'update': {
                ...
                await interaction.reply({ content: '✅ Profile updated.', flags: MessageFlags.Ephemeral });
                break;
            }
            case 'addwallet': {
                const address = interaction.options.getString('address');
                await ProfileService.addWallet(userId, address);
                await interaction.reply({ content: `✅ Wallet ${address} added.`, flags: MessageFlags.Ephemeral });
                break;
            }
            case 'removewallet': {
                const address = interaction.options.getString('address');
                await ProfileService.removeWallet(userId, address);
                await interaction.reply({ content: `✅ Wallet ${address} removed.`, flags: MessageFlags.Ephemeral });
                break;
            }
            case 'view': {
                const profile = await ProfileService.getProfile(userId);
                if (!profile) {
                    await interaction.reply({ content: '❌ No profile found. Use /profile create first.', flags: MessageFlags.Ephemeral });
                    return;
                }
            ...
            case 'reset': {
                ...
                await interaction.reply({ content: `✅ Password for <@${target.id}> has been reset.`, flags: MessageFlags.Ephemeral });
                break;
            }
        }
    } catch (error) {
...
function buildProfileEmbed(profile) {
    ...
    const rankProgress = profile.getRankProgress();
    const nextRank = profile.getNextRank();
    
    // Create progress bar
    const progressBarLength = 20;
    const filledBars = Math.floor((rankProgress.percentage / 100) * progressBarLength);
    ...
    if (rankProgress.isMaxRank) {
        progressText = `**${rankProgress.current} XP** - Maximum Rank!`;
    } else {
        progressText = `**${rankProgress.current}** / **${rankProgress.needed} XP**\n${progressBar} ${rankProgress.percentage}%\nNext: **${nextRank}**`;
    }

    const embed = new EmbedBuilder()
        .setTitle(`👤 **${profile.rank}** Player Profile`)
        .setColor(profile.getRankColor())
        .addFields([
            { 
            ...
            { name: '⚔️ Monsters Defeated', value: profile.stats.totalMonstersDefeated.toString(), inline: true },
            { name: '🏅 Achievements', value: profile.stats.achievements.length.toString(), inline: true }
        ])
        .setFooter({ text: 'XP is earned through quests and achievements' })
        .setTimestamp();
    return embed;
} 