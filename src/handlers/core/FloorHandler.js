...
import { getScalingInfo } from '../../utils/floorScaling.js';
...
class FloorHandler {
    ...
    static async showFloor(interaction, gameState) {
        try {
            ...
            const scalingInfo = getScalingInfo(currentFloor);
            
            // Get player hero info
            const playerHero = gameState.selectedHero;
            
            // Build description with scaling info for higher floors
            let description = `${floorDescription}\n\n**Explorations:** ${explorationCount}/${maxExplorations}`;
            
            // Add scaling information for floors 21+ or when at cap
            if (currentFloor >= 21 || scalingInfo.isAtCap) {
                description += `\n\n${scalingInfo.message}`;
            }
            ...
            // Check if we're transitioning from dungeon entrance - use embedHistory to create new message
            // Otherwise use update to modify existing floor message
            if (gameState.currentScreen === 'dungeon_entrance') {
    ...
    static async handleSelection(interaction, gameState, selectedValue) {
        try {
            // Debug logging to see what selectedValue actually is
            logger.info(`[FLOOR_HANDLER] handleSelection called with selectedValue: ${selectedValue}, type: ${typeof selectedValue}`);
            ...
            switch (selectedValue) {
                ...
                default:
                    logger.warn(`Unknown floor selection: ${selectedValue}`);
                    await embedHistory.updateWithHistory(interaction, {
     ...
     */
    static async handleFloorAction(interaction, selectedValue, gameState) {
        return await this.handleSelection(interaction, gameState, selectedValue);
    }
    ...
    /**
     * Handle heading to stairs (boss fight)
     */
    static async handleStairs(interaction, gameState) {
        try {
            const { BattleHandler } = await import('./BattleHandler.js');
            await BattleHandler.startBossBattle(interaction, gameState);
            
            logger.info(`User ${gameState.session.userId} headed to stairs on floor ${gameState.currentFloor}`);
            
        } catch (error) {
            logger.error('Error handling stairs:', error);
            await embedHistory.updateWithHistory(interaction, {
                content: 'Failed to proceed to stairs. Please try again.',
                embeds: [],
                components: []
            }, gameState.session.userId);
        }
