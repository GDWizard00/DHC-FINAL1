...
import { calculateGoldScaling, getEffectiveFloor } from '../utils/floorScaling.js';
...
export function getRandomChestRarity(floor = 1, luckModifier = 0) {
    ...
    // Adjust based on effective floor (higher floors = better chances) - capped at floor 500
    if (effectiveFloor >= 10) {
        ...
        if (effectiveFloor >= 20) {
            chances.rare += 10;
            chances.uncommon -= 10;
            if (effectiveFloor >= 30) {
                chances.epic += 5;
                chances.rare -= 5;
                if (effectiveFloor >= 40) {
                    chances.legendary += 3;
                    chances.epic -= 3;
...
export function generateChestRewards(chestData, floor = 1) {
    ...
    // Scale gold by floor (capped at floor 500)
    rewards.gold = calculateGoldScaling(baseGold, floor);
